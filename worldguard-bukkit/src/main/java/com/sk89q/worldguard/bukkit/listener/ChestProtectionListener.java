        ...
        if (player != null) {
            ...
            event.filter(target -> {
                if (wcfg.getChestProtection().isChest(BukkitAdapter.asBlockType(event.getEffectiveMaterial())) && wcfg.isChestProtected(BukkitAdapter.adapt(target.getBlock().getLocation()),
                    ...
                    sendMessage(event, player, ChatColor.DARK_RED + WorldGuard.getInstance().getMessages().get("protection.chest.spot-no-permission"));
                    return false;
                }
        ...
        if (player != null) {
            ...
            event.filter(target -> {
                if (wcfg.isChestProtected(BukkitAdapter.adapt(target.getBlock().getLocation()), localPlayer)) {
                    sendMessage(event, player, ChatColor.DARK_RED + WorldGuard.getInstance().getMessages().get("protection.chest.protected"));
                    return false;
                }
        ...
        if (wcfg.signChestProtection) {
            if ("[Lock]".equalsIgnoreCase(event.getLine(0))) {
                if (wcfg.isChestProtectedPlacement(BukkitAdapter.adapt(event.getBlock().getLocation()), WorldGuardPlugin.inst().wrapPlayer(player))) {
                    player.sendMessage(ChatColor.DARK_RED + WorldGuard.getInstance().getMessages().get("protection.chest.not-owner-adjacent"));
                ...
                if (!Tag.STANDING_SIGNS.isTagged(event.getBlock().getType())) {
                    player.sendMessage(ChatColor.RED
                            + WorldGuard.getInstance().getMessages().get("protection.chest.sign-must-be-post"));
                ...
                if (!player.getName().equalsIgnoreCase(event.getLine(1))) {
                    player.sendMessage(ChatColor.RED
                            + WorldGuard.getInstance().getMessages().get("protection.chest.first-line-name"));
                ...
                if (below == Material.TNT || below == Material.SAND
                    ...
                    player.sendMessage(ChatColor.RED
                            + WorldGuard.getInstance().getMessages().get("protection.chest.unsafe-block"));
                ...
                player.sendMessage(ChatColor.YELLOW
                        + WorldGuard.getInstance().getMessages().get("protection.chest.now-protected"));
            }
        } else if (!wcfg.disableSignChestProtectionCheck) {
            if ("[Lock]".equalsIgnoreCase(event.getLine(0))) {
                player.sendMessage(ChatColor.RED
                        + WorldGuard.getInstance().getMessages().get("protection.chest.disabled"));
