    ...
    protected ActionResult applyNonRepeated(BlacklistEvent event, boolean silent) {
        ...
        if (event.getPlayer() != null) {
            if (message != null) {
                ...
                event.getPlayer().printError(TextComponent.of(WorldGuard.getInstance().getMessages()
                        .format("blacklist.not-allowed", "action", event.getDescription()))
                        .append(event.getTarget().getFriendlyNameComponent()).append(TextComponent.of(".")));
            }
        }
