        ...
        if (blockedEffect != null) {
            ...
            if (player != null) {
                ...
                player.sendMessage(ChatColor.RED + WorldGuard.getInstance().getMessages()
                        .format("protection.potions.arrows-disabled", "effect", blockedEffect.getName()));
            }
                ...
                if (player != null) {
                    if (getPlugin().hasPermission(player, "worldguard.override.potions")) {
                        if (wcfg.blockPotionsAlways && (item.getType() == Material.SPLASH_POTION
                                || item.getType() == Material.LINGERING_POTION)) {
                            player.sendMessage(ChatColor.RED + WorldGuard.getInstance().getMessages()
                                    .format("protection.potions.cant-throw", "effect", blockedEffect.getName()));
                            event.setCancelled(true);
                        }
                    } else {
                        player.sendMessage(ChatColor.RED + WorldGuard.getInstance().getMessages()
                                .format("protection.potions.disabled", "effect", blockedEffect.getName()));
                        event.setCancelled(true);
                    }
                } else {
                    event.setCancelled(true);
