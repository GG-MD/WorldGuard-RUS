...
public class RegionProtectionListener extends AbstractListener {
    ...
    private static String deny(String key) {
        return WorldGuard.getInstance().getMessages().get("protection.deny." + key);
    }
    ...
    public void onEntityMount(EntityMountEvent event) {
        ...
        if (!query.testBuild(BukkitAdapter.adapt(location), localPlayer, Flags.RIDE, Flags.INTERACT)) {
            ...
            tellErrorMessage(dummy, cause, vehicle.getLocation(), deny("ride-that"));
        }
    }
        ...
        if (vehicle instanceof Tameable && exited instanceof Player player && !Entities.isNPC(player)) {
            ...
            if (!isWhitelisted(Cause.create(player), vehicle.getWorld(), false)) {
                ...
                if (!query.testBuild(BukkitAdapter.adapt(location), localPlayer, Flags.RIDE, Flags.INTERACT)) {
                    ...
                    if (lastTime == null || now - lastTime >= LAST_MESSAGE_DELAY) {
                        player.sendMessage("" + ChatColor.GOLD + WorldGuard.getInstance().getMessages().get("protection.disembark.warning")
                                + ChatColor.GRAY + WorldGuard.getInstance().getMessages().get("protection.disembark.detail"));
