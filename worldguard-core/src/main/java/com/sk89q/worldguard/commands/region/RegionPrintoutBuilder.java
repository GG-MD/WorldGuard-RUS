...
public class RegionPrintoutBuilder implements Callable<TextComponent> {
    ...
    public void appendBasics() {
        ...
        builder.append(TextComponent.of(messages.get("commands.region.info.region-label"), TextColor.BLUE));
        ...
        builder.append(TextComponent.of(messages.get("commands.region.info.type-prefix"), TextColor.GRAY));
        builder.append(TextComponent.of(region.getType().getName()));

        builder.append(TextComponent.of(messages.get("commands.region.info.priority-prefix"), TextColor.GRAY));
    ...
    public void appendFlags() {
        builder.append(TextComponent.of(WorldGuard.getInstance().getMessages()
                .get("commands.region.info.flags-label"), TextColor.BLUE));
    ...
    public void appendFlagsList(boolean useColors) {
        ...
        for (Flag<?> flag : WorldGuard.getInstance().getFlagRegistry()) {
            ...
            if (perms != null && perms.maySetFlag(region, flag)) {
                flagText = flagText.hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(messages.get("commands.region.info.click-set-flag"))))
        ...
        if (!hasFlags) {
            TextComponent noFlags = TextComponent.of(messages.get("commands.region.info.none"), useColors ? TextColor.RED : TextColor.WHITE);
            builder.append(noFlags);
        }

        if (perms != null && perms.maySetFlag(region)) {
            builder.append(TextComponent.space())
                    .append(TextComponent.of(messages.get("commands.region.info.flags-button"), useColors ? TextColor.GREEN : TextColor.GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(messages.get("commands.region.info.click-set-a-flag"))))
        ...
        while (it.hasPrevious()) {
            ...
            if (perms != null && perms.mayLookup(cur)) {
                builder.append(TextComponent.of(cur.getId(), useColors ? TextColor.GREEN : TextColor.WHITE)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(messages.get("commands.region.info.click-for-info"))))
            ...
            if (!cur.equals(region)) {
                builder.append(TextComponent.of(messages.get("commands.region.info.parent-prefix"), useColors ? TextColor.GRAY : TextColor.WHITE));
            ...
            if (last != null && cur.equals(region) && perms != null && perms.maySetParent(cur, last)) {
                builder.append(TextComponent.space());
                builder.append(TextComponent.of(messages.get("commands.region.info.unlink-button"), TextColor.RED)
                        .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(messages.get("commands.region.info.click-unlink-parent"))))
    ...
    public void appendDomain() {
        MessageBundle messages = WorldGuard.getInstance().getMessages();
        builder.append(TextComponent.of(messages.get("commands.region.info.owners-label"), TextColor.BLUE));
        ...
        builder.append(TextComponent.of(messages.get("commands.region.info.members-label"), TextColor.BLUE));
    ...
    private void addDomainString(DefaultDomain domain, String addCommand, String removeCommand) {
        MessageBundle messages = WorldGuard.getInstance().getMessages();
        if (domain.size() == 0) {
            builder.append(ErrorFormat.wrap(messages.get("commands.region.info.none")));
        } else {
            if (perms != null) {
        ...
        if (addCommand != null) {
            builder.append(TextComponent.space().append(TextComponent.of(messages.get("commands.region.info.add-button"), TextColor.GREEN)
                            .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(messages.get("commands.region.info.click-add"))))
                            .clickEvent(ClickEvent.of(ClickEvent.Action.SUGGEST_COMMAND,
                                    "/rg " + addCommand + " -w \"" + world + "\" " + region.getId() + " "))));
        }
        if (removeCommand != null && domain.size() > 0) {
            builder.append(TextComponent.space().append(TextComponent.of(messages.get("commands.region.info.rem-button"), TextColor.RED)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(messages.get("commands.region.info.click-remove"))))
                    .clickEvent(ClickEvent.of(ClickEvent.Action.SUGGEST_COMMAND,
                            "/rg " + removeCommand + " -w \"" + world + "\" " + region.getId() + " "))));
            builder.append(TextComponent.space().append(TextComponent.of(messages.get("commands.region.info.clr-button"), TextColor.RED)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(messages.get("commands.region.info.click-clear"))))
                    .clickEvent(ClickEvent.of(ClickEvent.Action.SUGGEST_COMMAND,
    ...
    public void appendBounds() {
        ...
        builder.append(TextComponent.of(messages.get("commands.region.info.bounds-label"), TextColor.BLUE));
        TextComponent bound = TextComponent.of(" " + min + " -> " + max, TextColor.YELLOW);
        if (perms != null && perms.maySelect(region)) {
            bound = bound
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(messages.get("commands.region.info.click-select"))))
        ...
        if (teleFlag != null && perms != null && perms.mayTeleportTo(region)) {
            builder.append(TextComponent.space().append(TextComponent.of(messages.get("commands.region.info.teleport-button"), TextColor.GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of(messages.get("commands.region.info.click-teleport")).append(TextComponent.newline()).append(
            ...
            builder.append(TextComponent.space().append(TextComponent.of(messages.get("commands.region.info.center-teleport-button"), TextColor.GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of(messages.get("commands.region.info.click-center-teleport"))))
                    .clickEvent(ClickEvent.of(ClickEvent.Action.RUN_COMMAND,
    ...
    private void appendPriorityComponent(ProtectedRegion rg) {
        ...
        if (perms != null && perms.maySetPriority(rg)) {
            builder.append(TextComponent.of(content, TextColor.GOLD)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of(WorldGuard.getInstance().getMessages().get("commands.region.info.click-change"))))
    ...
    private void appendRegionInformation() {
        ...
        if (cache != null && perms == null) {
            builder.append(SubtleFormat.wrap(WorldGuard.getInstance().getMessages().get("commands.region.info.last-seen-note")));
        }
    }

    @Override
    public TextComponent call() {
        MessageBox box = new MessageBox(WorldGuard.getInstance().getMessages().get("commands.region.info.title"), builder);
