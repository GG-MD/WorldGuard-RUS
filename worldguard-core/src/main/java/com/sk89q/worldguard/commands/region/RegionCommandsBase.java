...
class RegionCommandsBase {
        ...
        if (args.hasFlag(flag)) {
            ...
            if (sender instanceof LocalPlayer) {
                ...
                throw new CommandException(WorldGuard.getInstance().getMessages()
                        .format("commands.region.base.specify-world", "flag", String.valueOf(flag)));
            }
        }
    }
    ...
    protected static String checkRegionId(String id, boolean allowGlobal) throws CommandException {
        ...
        if (!ProtectedRegion.isValidId(id)) {
            throw new CommandException(
                    messages.format("commands.region.base.invalid-name", "id", id));
        }

        if (!allowGlobal && id.equalsIgnoreCase("__global__")) { // Sorry, no global
            throw new CommandException(
                    messages.get("commands.region.base.global-not-allowed"));
        }
    ...
    protected static ProtectedRegion checkExistingRegion(RegionManager regionManager, String id, boolean allowGlobal) throws CommandException {
        ...
        if (region == null) {
            ...
            throw new CommandException(WorldGuard.getInstance().getMessages()
                    .format("commands.region.base.not-found", "id", id));
        }
    ...
    protected static Region checkSelection(Actor actor) throws CommandException {
        ...
        try {
            ...
            throw new CommandException(WorldGuard.getInstance().getMessages()
                    .get("commands.region.base.select-area-first"));
        }
    }
    ...
    protected static void checkRegionDoesNotExist(RegionManager manager, String id, boolean mayRedefine) throws CommandException {
        if (manager.hasRegion(id)) {
            MessageBundle messages = WorldGuard.getInstance().getMessages();
            throw new CommandException(messages.get("commands.region.base.already-exists")
                    + (mayRedefine ? messages.format("commands.region.base.already-exists-redefine", "id", id) : ""));
        }
    }
    ...
    protected static RegionManager checkRegionManager(World world) throws CommandException {
        ...
        if (!WorldGuard.getInstance().getPlatform().getGlobalStateManager().get(world).useRegions) {
            throw new CommandException(messages.get("commands.region.base.regions-disabled"));
        }

        RegionManager manager = WorldGuard.getInstance().getPlatform().getRegionContainer().get(world);
        if (manager == null) {
            throw new CommandException(messages.get("commands.region.base.data-failed"));
        }
        return manager;
    }
    ...
    protected static ProtectedRegion checkRegionFromSelection(Actor actor, String id) throws CommandException {
        ...
        if (selection instanceof Polygonal2DRegion) {
            ...
            throw new CommandException(WorldGuard.getInstance().getMessages()
                    .get("commands.region.base.only-cuboid-poly"));
        }
    }
    ...
    protected static void warnAboutSaveFailures(Actor sender) {
        ...
        if (!failures.isEmpty()) {
            ...
            sender.print(TextComponent.of(WorldGuard.getInstance().getMessages()
                    .format("commands.region.base.save-failing", "worlds", failingList), TextColor.GOLD));
        }
    }
    ...
    protected static void informNewUser(Actor sender, RegionManager manager, ProtectedRegion region) {
        if (manager.size() <= 2) {
            MessageBundle messages = WorldGuard.getInstance().getMessages();
            sender.print(SubtleFormat.wrap(messages.get("commands.region.base.new-user-prefix"))
                            .append(TextComponent.of(messages.format("commands.region.base.new-user-command", "id", region.getId()), TextColor.AQUA))
                            .append(TextComponent.of(messages.get("commands.region.base.new-user-suffix"), TextColor.GRAY)));
        }
    }
    ...
    protected static boolean checkSpawnOverlap(Actor sender, World world, ProtectedRegion region) {
        ...
        if (spawn != null) {
            if (!spawn.getIntersectingRegions(ImmutableList.of(region)).isEmpty()) {
                MessageBundle messages = WorldGuard.getInstance().getMessages();
                sender.print(ErrorFormat.wrap(messages.get("commands.region.base.spawn-overlap-warning"))
                        .append(TextComponent.of(messages.get("commands.region.base.spawn-overlap-detail"), TextColor.WHITE)));
                return true;
            }
        }
        return false;
    ...
    protected static void setPlayerSelection(Actor actor, ProtectedRegion region, World world) throws CommandException {
        ...
        if (selector != null) {
            ...
            actor.print(messages.format("commands.region.base.selected", "type", region.getType().getName()));
        } else {
            throw new CommandException(messages.format("commands.region.base.cannot-select",
